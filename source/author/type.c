...
LLVMTypeRef get_type(author_t *author, MuonType *root) {
  ...
  Attitude this = {root, 0}, next;
  do {
    while (!attitude_is(next = type_scan3(author->inductor, this), NULL, 0)) {
      assert(next.type->id < author->type_length);

      LLVMTypeRef answer = author->type_to_type[next.type->id];
      if (answer != NULL)
        continue;

      this = type_continue(this, next);
    }

    LLVMTypeRef answer;
    if ((answer = type_emit(author, this.type)) == NULL)
      return NULL;
    author->type_to_type[this.type->id] = answer;
  } while (!attitude_is(this = type_return(this), NULL, 0));
