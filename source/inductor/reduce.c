...
const void *reduce_type_to_join(induce_t *induce, MuonVariableType *target) {
  ...
  for (type_edge_t *edge; (edge = universe_next(&it)) != NULL;) {
    ...
    if ((next_variable = muon_type_cast(edge->source, next_variable)) == NULL)
      continue;
  ...
  switch ON_ABSTRACT_OBJECT(coercion) {
    ...
    case IS_COERCION(MuonVarianceCoercion * nominate(variance_coercion)) {
      ...
      if ((result = variance_coercion_activate(
               allocation, variance_coercion->as_coercion.target))
          == NULL)
        return NULL;
      return &result->as_coercion;
    }
    ...
    case IS_COERCION(MuonUnjoinCoercion * nominate(unjoin_coercion)) {
      ...
      if ((result = unjoin_coercion_activate(
               allocation, unjoin_coercion->as_coercion.target))
          == NULL)
        return NULL;
      return &result->as_coercion;
    }
    ...
    case IS_COERCION(MuonMeetCoercion * nominate(meet_coercion)) {
      ...
      if ((result = meet_coercion_activate(
               allocation, meet_coercion->as_coercion.target))
          == NULL)
        return NULL;
      return &result->as_coercion;
    }
