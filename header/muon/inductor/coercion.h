...
#define MU_EACH_COERCION_KIND(emit, ...) \
  emit(id, ID, Id __VA_OPT__(,) __VA_ARGS__) \
  emit(edge, EDGE, Edge __VA_OPT__(,) __VA_ARGS__) \
  emit(slot, SLOT, Slot __VA_OPT__(,) __VA_ARGS__) \
  emit(indirect, INDIRECT, Indirect __VA_OPT__(,) __VA_ARGS__) \
  emit(variance, VARIANCE, Variance __VA_OPT__(,) __VA_ARGS__) \
  emit(unscheme, UNSCHEME, Unscheme __VA_OPT__(,) __VA_ARGS__) \
  emit(join, JOIN, Join __VA_OPT__(,) __VA_ARGS__) \
  emit(unjoin, UNJOIN, Unjoin __VA_OPT__(,) __VA_ARGS__) \
  emit(meet, MEET, Meet __VA_OPT__(,) __VA_ARGS__) \
  emit(unmeet, UNMEET, Unmeet __VA_OPT__(,) __VA_ARGS__)
