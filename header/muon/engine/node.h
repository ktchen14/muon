 ...
 *
 *   MuonAccessExpr *expr;
 *   if ((expr = muon_node_cast(abstract_node, expr)) == NULL)
 *     return ...;
 * @endcode
 *
...
struct MuonNodeDebugArgs {
  _Bool id;   ///< Whether to emit the id of each node
  int indent; ///< Amount of indentation

  /// Return the type of the @a node
  MuonType *(*type)(MuonNode *node, const void *data);
...
/// Emit debugging information on the abstract @a node to the debug stream
#define muon_node_debug(node, ...) \
  muon_node_debug((node), (struct MuonNodeDebugArgs) {__VA_ARGS__})
